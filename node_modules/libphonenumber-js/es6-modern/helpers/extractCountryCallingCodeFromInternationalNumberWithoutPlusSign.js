...
export default function extractCountryCallingCodeFromInternationalNumberWithoutPlusSign(number, country, defaultCountry, defaultCallingCode, metadata) {
  const countryCallingCode = country || defaultCountry ? getCountryCallingCode(country || defaultCountry, metadata) : defaultCallingCode;
  if (number.indexOf(countryCallingCode) === 0) {
    ...
    } = extractNationalNumber(possibleShorterNumber, country, metadata);
    const {
      nationalNumber
    } = extractNationalNumber(number, country, metadata);
